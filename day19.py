...
""".strip().split("\n\n")
workflow_dict = {k: [i.split(":") if ":" in i else [None, i] for i in v.split(",")]
                 for k, v in (line.rstrip("}").split("{") for line in workflow_lines.split())}
part_list = [eval(f"dict({line[1:-1]})") for line in part_lines.split()]


# PART 1
def filter_parts(parts, workflows):
    for part in parts:
        wf_name = "in"
        while wf_name not in ["R", "A"]:
            for cond, wf_name in workflows[wf_name]:
                if cond and eval(cond, None, part):
                    break
        if wf_name == "A":
            yield part
...
# PART 2
def cond_as_tuple(cond: str):
    return cond[0], cond[1], int(cond[2:])


def gather_conditions(workflows: dict[str, list[tuple[str, str]]], wf_name="in"):
    if wf_name in ["R", "A"]:
        yield [wf_name == "A"]
        return
    failed_conds = []
    for cond, goto_wf in workflows[wf_name]:  # cond is None in final iteration
        # with cond as True, gather all condition combinations after GOTO
        for subcondition in gather_conditions(workflows, goto_wf):
            yield failed_conds + ([cond_as_tuple(cond)] if cond else []) + subcondition
        # negate cond and proceed with following rules in the workflow
        if cond:
            n, sign, num = cond_as_tuple(cond)
            failed_conds.append((n, "<" if sign == ">" else ">", num + (1 if sign == ">" else -1)))


def accepted_combinations(workflows):
    accum = 0
    for condset in gather_conditions(workflows):
        if condset[-1]:  # True <=> conditions for accepted parts
            combs = []
            for k, g in groupby(sorted(condset[:-1], key=itemgetter(0)), key=itemgetter(0)):
                g = list(g)
                gmax = min((t[2] for t in g if t[1] == "<"), default=4001)
                gmin = max((t[2] for t in g if t[1] == ">"), default=0)
                combs.append(max(0, gmax - gmin - 1))
            combs += [4000] * (4 - len(combs))  # add 4000 for each omitted rating category
            accum += prod(combs)
    return accum


if __name__ == '__main__':
    filtered = filter_parts(part_list, workflow_dict)
    print(f"Sum of ratings of accepted parts: {sum(sum(part.values()) for part in filtered)}")
    print(f"Total acceptable combinations: {accepted_combinations(workflow_dict)}")

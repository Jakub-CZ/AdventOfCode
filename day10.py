...
""".strip()
X_MAX = input_str.find("\n")
sketch = input_str.replace("\n", "")  # will process as 1-D array
start_pos = sketch.find("S")
...
connection_pairs = {"N": "S", "S": "N", "E": "W", "W": "E"}
offset = {"N": -X_MAX, "S": +X_MAX, "E": +1, "W": -1}
distances = [None] * len(sketch)  # type: list[int|None]


def get_neighbor_idx(idx, direction):
    if (idx % X_MAX, direction) == (0, "W"):
        return None  # out of bounds
    if (idx % X_MAX, direction) == (X_MAX - 1, "E"):
        return None  # out of bounds
    if 0 <= (neighbor_idx := idx + offset[direction]) < len(sketch):
        return neighbor_idx
    return None  # out of bounds


# PART 1
def process_neighbors(idx: int, dist):
    neigh_connections = []  # list of pipes connected to current pipe
    if distances[idx] is not None:
        return []  # already visited
    distances[idx] = dist
    for dir_src in pipe_connections[sketch[idx]]:
        if (nidx := get_neighbor_idx(idx, dir_src)) is None:
            continue  # neighbor would be out of bounds
        if connection_pairs[dir_src] not in pipe_connections[sketch[nidx]]:
            continue  # pipes don't connect (e.g. "N" -> north neighbor should contain "S" in its connections)
        neigh_connections.append(nidx)
...
def process_from_start():
    nodes = [start_pos]
    for i in count():
        nodes = list(chain.from_iterable([process_neighbors(idx, i) for idx in nodes]))
        if not nodes:
            return max(filter(None, distances))


# PART 2

# As we follow the pipeline, we label all tiles that are on one side of the trajectory (e.g. left)
left_orthogonal = {"N": "W", "S": "E", "E": "N", "W": "S"}
FREE = " "
INSIDE = "<"  # left of trajectory; might be 'outside' depending on the trajectory direction


def follow_path(idx: int, _dir):
    output = [FREE] * len(sketch)  # type: list[str]
    output[idx] = "|"

    def mark_left(i):
        if i is not None and output[i] != "|":
            output[i] = INSIDE

    while True:
        # mark "left of trajectory" both before and after we move (both because of corners)
        mark_left(get_neighbor_idx(idx, left_orthogonal[_dir]))
        idx = get_neighbor_idx(idx, _dir)
        if idx is None:
            return None  # out of bounds
        mark_left(get_neighbor_idx(idx, left_orthogonal[_dir]))
        if output[idx] == "|":
            return output  # we've finished the whole loop
        output[idx] = "|"
        if connection_pairs[_dir] not in pipe_connections[sketch[idx]]:
            return None  # pipes don't connect
        # update direction; remove the direction which we just came from
        _dir = pipe_connections[sketch[idx]].replace(connection_pairs[_dir], "")
        assert len(_dir) == 1, f"ERROR: {_dir=}"


def fill_loop(output):
    if not output:
        return None
    marked = [m.start() for m in re.finditer("<", "".join(output))]
    while marked:
        idx = marked.pop()
        neighbors = [get_neighbor_idx(idx, d) for d in offset.keys()]
        if any(n is None for n in neighbors):
            return None  # we're at the boundary, so this can't be enclosed within the loop
        for n in neighbors:
            if output[n] == " ":
                output[n] = "<"
                marked.append(n)
...
def get_enclosed_count():
    for _dir in pipe_connections["S"]:
        if out := fill_loop(follow_path(start_pos, _dir)):
            return out.count("<")


if __name__ == '__main__':
    print(f"Steps to farthest: {process_from_start()}")
    print(f"Tiles are enclosed by the loop: {get_enclosed_count()}")

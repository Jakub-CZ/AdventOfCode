...
def is_possible_draw(draw_str: str) -> bool:
    for amnt_color in draw_str.split(", "):
        amount_drawn, color_drawn = amnt_color.split(" ")
        if bag[color_drawn] < int(amount_drawn):
            return False
    return True


def get_possible_game_id(game_str: str) -> Optional[int]:  # None if game is not possible
    game_label, draws = game_str.split(": ")
    game_id = int(game_label.split(" ")[1])
    if all(map(is_possible_draw, draws.split("; "))):
        return game_id


# PART 2
def get_max_per_color(game_str: str) -> dict[str, int]:
    amnts_colors = game_str.split(": ")[1].replace(";", ",").split(", ")
    grp_by_color = groupby(sorted(map(str.split, amnts_colors), key=itemgetter(1)), key=itemgetter(1))
    return {k: max(map(int, map(itemgetter(0), g))) for k, g in grp_by_color}


if __name__ == '__main__':
    print(f"Sum of all possible game IDs: {sum(filter(None, map(get_possible_game_id, games)))}")
    print(f"Sum of powers of sets of cubes: {sum(math.prod(grp.values()) for grp in map(get_max_per_color, games))}")

...
# PART 1
BLOCK_WITH_DIGITS = re.compile(r"\d(.*\d)?")  # match even if there's only one digit


def get_two_digits(line: str) -> int:
    block = BLOCK_WITH_DIGITS.search(line).group()
...
digit_or_word = rf"(\d|{'|'.join(digit_map.keys())})"
BLOCK_WITH_DIGITS_OR_WORDS = re.compile(rf"{digit_or_word}(?:.*{digit_or_word})?")


def get_two_digits_or_words(line: str) -> int:
    m = BLOCK_WITH_DIGITS_OR_WORDS.search(line)
    # map the matched word to digit, or just convert the digit to int
    a = digit_map.get(left := m.group(1)) or int(left)
    # process the 2nd group; if there was only one match group, use it as the rightmost element, too
    right = m.group(2) or left
    b = digit_map.get(right) or int(right)

...
# PART 1
def get_columns_with_symbols(symbol_columns: dict[int, list[int]], line_num: int) -> set[int]:
    # return indices of columns where symbols exist either on the given line or on one of the neighbouring lines
    out = set(symbol_columns[line_num])
    if line_num > 0:
        out = out.union(symbol_columns[line_num - 1])  # add previous line
    out = out.union(symbol_columns.get(line_num + 1) or [])  # add next line
    return out


def get_part_numbers(lines: list[str]):
    # dictionary: line index -> list of indices where a symbol occurs on the line.
    symbols = {n: [match.start() for match in re.finditer(r"[^\d.]", line)] for n, line in enumerate(lines)}

    for n, line in enumerate(lines):
        cols = get_columns_with_symbols(symbols, n)
        for match in re.finditer(r"\d+", line):
            # Match.end() points AFTER the end, not AT the (like in list slicing)
            if all(c < match.start() - 1 or c > match.end() for c in cols):
                continue  # not adjacent to any symbol
            yield int(match.group())


# PART 2
def get_gear_parts(lines: list[str]):
    gears = defaultdict(list)  # dictionary: gear candidate -> list of adjacent part numbers
    # list of coordinates of symbols '*'
    stars = [(n, match.start()) for n, line in enumerate(lines) for match in re.finditer(r"\*", line)]

    for n, line in enumerate(lines):
        for match in re.finditer(r"\d+", line):
            m_left, m_right = match.start() - 1, match.end()
            for g_row, g_col in stars:  # find ALL gears that neighbor with this number
                if n - 1 <= g_row <= n + 1 and m_left <= g_col <= m_right:
                    gears[g_row, g_col].append(int(match.group()))
    return filter(lambda nums: len(nums) == 2, gears.values())  # only gears with two adjacent parts


if __name__ == '__main__':
    print(f"Sum of all part numbers: {sum(get_part_numbers(schema))}")
    print(f"Sum of all gear ratios: {sum(map(math.prod, get_gear_parts(schema)))}")
